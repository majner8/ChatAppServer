...
@Component
public interface JwtTokenInterfaces {
	 ...
	 **/
	default TokenDTO generateAuthorizationToken(String userId,Long Version,String deviceID,boolean isUserActive,Duration tokenValidity,long logId) {
	...
	default DecodedJWT tokenAuthorizationValidation(HttpServletRequest request) {
	...
	default String generateDeviceToken(String deviceId) {
	...
	default DecodedJWT tokenDeviceValidation(HttpServletRequest request) {
		Log4j2.log.debug(Log4j2.LogMarker.Security.getMarker(),"Trying verify token");
		String header=request.getHeader(HttpHeaders.AUTHORIZATION); 
